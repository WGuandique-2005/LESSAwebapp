@php
    // Cargar datos de Abecedario y tomar hasta 8 aleatorios (para 16 cartas)
    ...
    $abecedarioJuego = array_slice($abecedario, 0, min(8, count($abecedario)));

    // Cada elemento produce 2 cartas: imagen (seña) y texto (nombre)
    $cartas = [];
    foreach ($abecedarioJuego as $s) {
...
<head>
    ...
    <title>Memorama — Abecedario</title>
...
<body>
        ...
        <div class="container">
            <header class="mem-header">
                <div class="title-block">
                    <h1 class="mem-title">Memorama — Abecedario LESSA</h1>
                    <p class="mem-sub">Encuentra la pareja: **seña ↔ nombre**. Toca o presiona Enter para voltear.</p>
                </div>
                <div class="stats-block">
                    <div class="stat-item">
                        <i class="fas fa-check-circle success"></i>
                        <span>Parejas: <strong id="pairsFound">0</strong> / {{ count($abecedarioJuego) }}</span>
                    </div>
                    <div class="stat-item">
            ...
            <form id="completeForm" method="POST" action="{{ route('lecciones.abecedario.memorama.complete') }}">
                @csrf
    ...
    <div id="endGameModal" class="modal-end-game">
        <div class="modal-content">
            ...
            <div class="modal-actions">
                ...
                <a href="{{ route('nivel.abecedario') }}" class="btn btn-primary">
    ...
    <script>
        (function() {
            ...
            let pairsFound = 0;
            let totalErrors = 0; 
            const totalPairs = {{ count($abecedarioJuego) }};
            const MAX_HINTS = 3;
            let hintsUsed = 0;
            ...
            function showGameResults() {
                ...
                if (errors <= 0) {
                    points = 10;
                    title = '¡PERFECTO! 🤩';
                    message = '¡Increíble! No tuviste ningún error. ¡Dominio total del abecedario!';
                } else if (errors <= 4) {
                    points = 5;
                    title = '¡MUY BIEN! 👍';
