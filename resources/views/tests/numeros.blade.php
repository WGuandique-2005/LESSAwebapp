    ...
    <div class="game-frame">
        ...
        <div class="card compact-card">
            <div class="card-body">
                    ...
                    <div class="sena-column" aria-label="Señas (destinatarios)">
                        ...
                        <div class="sena-grid" id="senaGrid">
                            @foreach($numerosJuego as $num)
                                <div class="sena-card drop-zone" data-id="{{ $num['id'] }}" tabindex="0" aria-dropeffect="none">
                                    <div class="sena-media">
                                        <img src="{{ $num['ruta'] }}" alt="{{ $num['nombre'] }}" class="sena-img" loading="lazy" draggable="false">
                                    </div>
                    ...
                    <div class="significados-column" aria-label="Significados (draggables)">
                        ...
                        <div class="significados-list" id="significadosList">
                            @foreach($significados as $sig)
                                <div class="significado-card draggable" draggable="true" data-id="{{ $sig['id'] }}" tabindex="0" role="button" aria-grabbed="false">
                ...
                <form id="completeForm" class="actions-row" method="POST" action="{{ route('lecciones.numeros.complete') }}">
                    @csrf
                    <input type="hidden" name="assigned" id="assignedInput" value="{}">
                    <button id="btnReset" class="btn-ghost" type="button">Reintentar</button>
                    <button id="btnComplete" class="btn-primary" type="submit" disabled aria-disabled="true" title="Completar lección">Completar</button>
                </form>
            </div>
        </div>
    </div>
...
<style>
    ...
    html,body{height:100%;margin:0;background:linear-gradient(180deg,var(--bg),#ffffff);-webkit-font-smoothing:antialiased;font-family:Poppins, system-ui, -apple-system, "Segoe UI", Roboto, Arial;}

    .game-wrap{display:flex;align-items:center;justify-content:center;padding:18px;min-height:100vh;box-sizing:border-box;}
    .game-frame{width:min(900px,96vw);display:flex;flex-direction:column;gap:14px;margin:0 auto;}

    .game-header{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:6px 8px;}
    .head-text{display:flex;flex-direction:column;gap:6px;}
    .title{font-size:1.15rem;margin:0;color:#0f172a;font-weight:800;}
    .subtitle{margin:0;font-size:0.92rem;color:var(--muted);}

    .progress-pill{position:relative;width:130px;height:30px;border-radius:999px;background:linear-gradient(90deg,#eef6ff,#fbfdff);border:1px solid rgba(10,30,80,0.04);overflow:hidden;display:flex;align-items:center;justify-content:center;padding:3px;box-shadow:var(--shadow-soft);}
    .progress-inner{position:absolute;left:0;top:0;bottom:0;width:0;background:linear-gradient(90deg,var(--accent),var(--accent-2));border-radius:999px;transition:width .25s ease;}
    .progress-label{position:relative;z-index:2;font-weight:700;color:#04203b;font-size:0.82rem;}

    .compact-card{background:var(--card);border-radius:var(--radius);box-shadow:var(--shadow-soft);border:1px solid rgba(13,27,62,0.04);overflow:hidden;}
    .card-body{padding:14px;}

    /* Play area layout */
    .play-area{display:flex;gap:18px;align-items:flex-start;justify-content:space-between;}
    .sena-column,.significados-column{flex:1;min-width:0;}
    .col-title{font-size:0.95rem;margin:0 0 8px 0;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:0.06em}

    /* Señas grid */
    .sena-grid{display:grid;grid-template-columns:repeat(1,1fr);gap:12px;}
    .sena-card{position:relative;display:flex;align-items:center;justify-content:center;flex-direction:column;padding:12px;background:linear-gradient(180deg,#fff,#fbfdff);border-radius:12px;border:1px solid rgba(10,25,60,0.04);box-shadow:0 6px 18px rgba(8,20,45,0.03);min-height:120px;text-align:center;transition:transform .12s ease,box-shadow .12s ease;}
    .sena-card:focus{outline:3px solid rgba(11,99,255,0.12);outline-offset:3px}
    .sena-media{display:flex;align-items:center;justify-content:center;width:100%}
    .sena-img{width:96px;height:96px;object-fit:contain;border-radius:8px;background:linear-gradient(180deg,#fff,#f7fbff);border:1px solid rgba(10,25,60,0.03)}
    .drop-target{min-height:44px;margin-top:10px;background:var(--glass);border-radius:10px;border:1px dashed rgba(90,100,120,0.08);display:flex;align-items:center;justify-content:center;padding:6px 8px;color:var(--muted);min-width:120px;transition:all .18s ease}

    /* Significados draggable */
    .significados-list{display:flex;flex-direction:column;gap:10px;align-items:stretch}
    .significado-card{background:#fff;border-radius:10px;padding:10px;border:1px solid rgba(10,25,60,0.06);box-shadow:0 6px 18px rgba(8,20,45,0.03);cursor:grab;transition:transform .12s ease,box-shadow .12s ease;display:flex;flex-direction:column;gap:6px}
    .significado-card:active{cursor:grabbing}
    .significado-card[aria-grabbed="true"]{opacity:0.7;transform:scale(.995)}
    .significado-nombre{font-weight:800;color:var(--accent)}

    /* states */
    .drop-zone.over{border-color:var(--accent-2);background:#eaf6ff}
    .drop-zone.acertado{background:#f2fbf4;border-color:rgba(22,163,74,0.12)}
    .drop-zone.incorrecto{background:#fff6f7;border-color:rgba(239,68,68,0.12);animation:shake .36s}
    .significado-card.hidden{display:none}

    @keyframes shake {0%{transform:translateX(0)}20%{transform:translateX(-6px)}40%{transform:translateX(6px)}60%{transform:translateX(-4px)}80%{transform:translateX(4px)}100%{transform:translateX(0)}}

    .actions-row{display:flex;gap:8px;justify-content:center;margin-top:12px;flex-wrap:wrap}
    .btn-ghost{background:transparent;border:1px solid rgba(10,25,60,0.06);color:#0b254a;padding:8px 12px;border-radius:10px;font-weight:700;cursor:pointer}
    .btn-primary{background:linear-gradient(90deg,var(--accent),var(--accent-2));color:#fff;border:none;padding:8px 14px;border-radius:10px;font-weight:800;cursor:pointer}
    ...
    @media (min-width: 880px){
        ...
        .sena-grid{grid-template-columns:repeat(2,1fr)}
        .significados-list{flex-direction:column}
    }

    @media (max-width: 879px){
        .play-area{flex-direction:column}
        .sena-grid{grid-template-columns:repeat(2,1fr)}
        .significados-list{flex-direction:row;flex-wrap:wrap;gap:8px}
        .significado-card{min-width:calc(50% - 8px)}
    }

    @media (max-width: 520px){
        .sena-grid{grid-template-columns:repeat(3,1fr)}
        .sena-img{width:78px;height:78px}
        .drop-target{min-width:100px}
        .significado-card{min-width:100%}
